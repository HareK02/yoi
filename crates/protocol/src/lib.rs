...
pub enum Method {
    ...
    /// Running or Paused Workers reject Submit; use Notify for advisory information
    /// that should be incorporated into work already in progress.
    ///
...
pub enum SubmissionDisposition {
    Started,
    /// Compatibility value for durable receipts created before Submit became
    /// Idle-only. New submissions are never accepted with this disposition.
    Queued,
}
...
pub enum Event {
    ...
    /// and a `Started` receipt follows the pending activation checkpoint; the
    /// corresponding [`Event::UserMessage`] marks the later history commit.
    /// `Queued` remains readable for receipts persisted by older runtimes.

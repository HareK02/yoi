...
use server_api::repository_openapi_typescript::{
    INPUT_PATH, RUNTIME_OUTPUT_PATH, generate_runtime_typescript,
};
...
fn run() -> Result<(), String> {
    ...
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Generate the bounded Runtime API TypeScript artifact.\n\n\
                     Usage: cargo run -q -p server-api --example generate_runtime_api_types -- [--check]"
                );
                return Ok(());
            }
    ...
    let output_path = root.join(RUNTIME_OUTPUT_PATH);
    let input = fs::read_to_string(&input_path)
        .map_err(|error| format!("failed to read {}: {error}", input_path.display()))?;
    let generated = generate_runtime_typescript(&input)
        .map_err(|error| format!("failed to generate Runtime API types: {error}"))?;

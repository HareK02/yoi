...
use server_api::repository_openapi_typescript::{
    INPUT_PATH, TICKET_BROWSER_OUTPUT_PATH, generate_ticket_browser_typescript,
};
...
fn run() -> Result<(), String> {
    ...
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Generate the bounded Ticket Browser API TypeScript artifact.\n\n\
                     Usage: cargo run -q -p server-api --example generate_ticket_api_types -- [--check]"
                );
                return Ok(());
            }
    ...
    let output_path = root.join(TICKET_BROWSER_OUTPUT_PATH);
    let input = fs::read_to_string(&input_path)
        .map_err(|error| format!("failed to read {}: {error}", input_path.display()))?;
    let generated = generate_ticket_browser_typescript(&input)
        .map_err(|error| format!("failed to generate Ticket Browser API types: {error}"))?;

...
use server_api::repository_openapi_typescript::{
    INPUT_PATH, WORKER_OUTPUT_PATH, generate_worker_typescript,
};
...
fn run() -> Result<(), String> {
    let check = parse_check_argument()?;
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input_path = root.join(INPUT_PATH);
    let output_path = root.join(WORKER_OUTPUT_PATH);
    let input = fs::read_to_string(&input_path)
        .map_err(|error| format!("failed to read {}: {error}", input_path.display()))?;
    let generated = generate_worker_typescript(&input)
        .map_err(|error| format!("failed to generate Worker API types: {error}"))?;
    if check {
...
fn parse_check_argument() -> Result<bool, String> {
    let mut check = false;
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Generate the bounded Worker API TypeScript artifact.\n\nUsage: cargo run -q -p server-api --example generate_worker_launch_api_types -- [--check]"
                );
                return Ok(true);
            }

            ...
            if refresh_changes.contains(McpListChangedKind::Tools) {
                contribution = contribution.with_diagnostic(FeatureDiagnostic::warning(
                    mcp_list_changed_startup_diagnostic(
                        &server_namespace,
                        &refresh_changes,
                        "using the refreshed tools/list for this registration; restart the Worker to refresh again because active-run tool schemas are not mutated after registration",
                    ),
                ));
            }
        }
...
fn provider_declaration(name: &str, version: Option<&str>) -> ProtocolProviderDeclaration {
    ...
    .with_description("MCP stdio server discovered at Worker startup")
}
...
fn mcp_list_changed_runtime_diagnostic(
    ...
    if snapshot.contains(McpListChangedKind::Tools) {
        policy.push(
            "model-visible MCP tool schemas are fixed for the active run; restart the Worker or start a new run to rediscover tools",
        );
    }
